...
def ticker(request, ticker):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			instance.save()
			return HttpResponseRedirect('/trade/dashboard/' + str(instance.league))
	else:
...
def dashboardLeague(request, league):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			league = form.cleaned_data['League_name']
			return HttpResponseRedirect('/trade/dashboard/' + str(league))
	else:
...
def createLeague(request):
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			instance.save()
			instance.players.add(request.user)
			return HttpResponseRedirect('/trade/leagues/')
	else:
